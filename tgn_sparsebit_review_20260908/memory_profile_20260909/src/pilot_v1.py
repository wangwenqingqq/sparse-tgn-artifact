...
def serialize(prof):
    ...
    for e in prof.events():
        rows.append(dict(id=e.id,name=e.name,thread=e.thread,fwd_thread=e.fwd_thread,
            seq=e.sequence_nr,scope=e.scope,device=str(e.device_type),start=e.time_range.start,end=e.time_range.end,
            parent=e.cpu_parent.id if e.cpu_parent is not None else None,is_async=e.is_async,
...
def main():
    ...
    for variant in ['direct','batch_store']:
        ...
        i.b.step(tr,40,'direct');tr.restore(ck)
