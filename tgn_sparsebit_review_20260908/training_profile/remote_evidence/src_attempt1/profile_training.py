...
def get_data(name):
    ...
    tensors = [torch.as_tensor(a, device='cuda', dtype=d) for a, d in
...
class Trainer:
    def __init__(self, name, bs):
        ...
        mem = TGNMemory(self.n, self.info['message_dim'], 100, 100,
                        ...
                        LastAggregator(), t_enc_grad=True).cuda()
        gnn = GraphAttentionEmbedding(100, 100, self.info['message_dim'], mem.time_enc).cuda()
        pred = NCNPredictor(100, 256, 1, 2).cuda()
        self.model = torch.nn.ModuleDict(dict(memory=mem, gnn=gnn, pred=pred))
        self.model.train()
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=1e-4)
        self.neighbor = LastNeighborLoader(self.n, size=10, device='cuda')
        self.assoc = torch.empty(self.n, dtype=torch.long, device='cuda')
        self.clock = Clock()
...
def run_window(tr, start, out, length=8, rounds=3):
    ...
    checks = []
    for variant in ['source', 'free_graph', 'free_relations']:
        ...
        torch.save(results, out/(label+'_'+variant+'_checks.pt'))
