...
for case in cases:
    ...
    for v in ['free_graph', 'free_relations']:
        ...
        item[v+'_meets_1p10'] = bool(checks['pass_all'] and ratio >= 1.10)
    for r in rows:
...
for name, data in [('bounds.csv', summary), ('phases.csv', phases)]:
    with (args.output/name).open('w') as f:
        ...
        w.writeheader()
        w.writerows(data)
lines = ['| 训练窗口 | keeper ms/步 | 免费建图上限 | 免费全部关系上限 | S 非零数/步 | 数值核对 |',
         '|---|---:|---:|---:|---:|---|']
for r in summary:
    lines.append(f"| {r['case']} | {r['keeper_ms']:.3f} | {r['free_graph_ratio']:.3f}× | {r['free_relations_ratio']:.3f}× | {r['mean_S_nnz']:.1f} | {'通过' if r['numeric_checks_pass'] else '失败：仅诊断'} |")
