...
for name,h in json.loads((EXP/'source_manifest_v2.json').read_text()).items():assert hashlib.sha256((ROOT/name).read_bytes()).hexdigest()==h,name
sys.path.insert(0,str(ROOT/'experiments/temporal_tc_20260907_cfree_gpu_screen/src'))
import run as r
import backend as cf
import torch,numpy as np
from explicit_v2 import identity,transpose,run
ref=cf.ref
