...
class Plan:
    def __init__(self,n,edges,targets,mode='list',checked=False):
...
def decode(model,x,edges,targets,mode='list',decay=False,time_info=None,library=False):
    p=Plan(len(x),edges,targets,mode,checked=True)
...
def pair(model,x,edges,targets,variant):
    ...
    if 'shared' in variant:
        p=Plan(len(x),edges,targets,mode,checked=True);y=p.consume(x,library=library)
