...
import numpy as np
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'output/analysis1';OUT.mkdir(parents=True,exist_ok=False)
V=['L-separate','B-separate','L-shared','B-shared','L-shared-library']
raw=[]
for p in range(5):
    rows=json.loads((ROOT/f'output/gpu1/bench{p}/samples.json').read_text());assert len(rows)==900
