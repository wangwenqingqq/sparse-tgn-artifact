...
phase correct "$PY" src/run.py --mode correct --output "output/$RUN_ID/correct"
...
for tool in memcheck racecheck synccheck initcheck; do
  phase "$tool" compute-sanitizer --tool "$tool" --error-exitcode 99 "$PY" src/run.py --mode sanitizer --output "output/$RUN_ID/$tool"
done
phase stress "$PY" src/run.py --mode stress --output "output/$RUN_ID/stress"
phase pair_check "$PY" src/run.py --mode pair-check --output "output/$RUN_ID/pair_check"
for i in 0 1 2 3 4; do
  phase "bench$i" "$PY" src/run.py --mode bench --process "$i" --output "output/$RUN_ID/bench$i"
done
