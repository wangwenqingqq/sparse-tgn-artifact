...
ax.set_xlabel('Paid batch-store baseline / precomputed-read oracle time')
ax.set_title(f"A. Idealized read-side opportunity: {s['speedup']['geomean']:.3f}x geometric mean\n9 paired rounds per window; 8 complete steps per interval; descriptive 95% bootstrap",loc='left',fontweight='bold')
ax.legend(frameon=False,loc='best')
paid=np.array([r['prefix']['batch_store']['timeline_ms_per_step'] for r in rows]);free=np.array([r['prefix']['free_cache_reads']['timeline_ms_per_step'] for r in rows])
bx.barh(y-.16,paid,height=.3,label='Paid lookup / unpack / cat',color='#4a899d');bx.barh(y+.16,free,height=.3,label='Oracle cursor and tensor references',color='#e2aa6d')
bx.set_yticks(y,labels);bx.invert_yaxis();bx.set_xlabel('Prefix ms/step in separate instrumented replay')
bx.set_title('B. Only the constant cache-read prefix is removed\nTime encoding, live-memory gathers, GRU, cache writes, backward and Adam remain paid',loc='left',fontweight='bold')
bx.legend(frameon=False,loc='lower right');bx.grid(axis='x',alpha=.2)
fig.savefig(OUT/'read_opportunity.png',dpi=180);fig.savefig(OUT/'read_opportunity.svg');plt.close(fig)
