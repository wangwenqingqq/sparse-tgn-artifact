...
def main():
    ...
    p.emit('opening',stage=args.stage,torch=torch.__version__,gpu=torch.cuda.get_device_name(),
        ...
        hashes={str(f):p.sha(f) for f in [Path(__file__),ROOT/'src/oracle.py',ROOT/'CONTRACT.md',o.PREV/'src/instrument.py',i.PREV/'src/bridge.py',d.OLD/'src/profile_training.py',Path(module.__file__)]})
