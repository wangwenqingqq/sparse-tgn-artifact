...
gpu = 'GPU-a149f5af-55ab-ce33-8d3d-371a7ae61dd2'
env = dict(os.environ, CUDA_VISIBLE_DEVICES=gpu, PYTHONDONTWRITEBYTECODE='1',
           PYTHONPATH=str(root/'deps')+':/home/data/wangxuran/factor_tgn_sptc_20260902/vendor/flash-tgn-artifact/python', OMP_NUM_THREADS='1', OPENBLAS_NUM_THREADS='1', MKL_NUM_THREADS='1',
           CUBLAS_WORKSPACE_CONFIG=':4096:8')
for key in list(env):
    if key.startswith('FLASH_TGN_'):env.pop(key)
env.update(FLASH_TGN_STRICT_FUSED='1',FLASH_TGN_ATTN_LOG='0' if args.stage=='timing' else '1',FLASH_TGN_PROFILE='0')
...
with open('/home/data/wangxuran/.locks/tgn_sptc_gpu3.lock', 'a+') as lock:
    try:
        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        g, a = snap('before')
        row = [r for r in g.splitlines() if gpu in r][0].split(',')
        assert int(row[-2]) <= 64 and int(row[-1]) == 0 and gpu not in a, 'GPU not idle'
        cmd = ['/home/data/wangxuran/isaacsim6/env/bin/python', str(root/'src/fullrun.py'),
               '--output', str(out/'artifacts'), '--stage', args.stage, '--rep', str(args.rep)]
