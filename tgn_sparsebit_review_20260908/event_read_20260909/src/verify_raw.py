...
def original_data(label):
    project=Path('/home/data/wangxuran/factor_tgn_sptc_20260902/project')
    if label.startswith('wikipedia_'):
        file=project/'experiments/temporal_tc_20260905_cooccurrence_gate0/data/wikipedia20k.npz'
        with np.load(file) as z:
            data=[z['src'].astype(np.int64),z['dst'].astype(np.int64),z['time'].astype(np.int64),z['edge'][1:].astype(np.float32)]
    else:
        file=project/'experiments/temporal_tc_20260905_joint_consumer_gate0/data/CollegeMsg.txt.gz'
        with gzip.open(file,'rt') as f:raw=np.loadtxt(f,dtype=np.int64)
        raw=raw[np.argsort(raw[:,2],kind='stable')][:20000]
        data=[raw[:,0],raw[:,1],raw[:,2]-raw[0,2],np.zeros((len(raw),1),np.float32)]
    return data,dict(path=str(file),sha256=hashlib.sha256(file.read_bytes()).hexdigest())


def event_check(label,records):
    data,info=original_data(label);_,batch,start=label.split('_');bs=int(batch[1:]);start=int(start[1:])
    result=[]
    for pos,row in enumerate(records):
        events=row['event_ids'].numpy();limit=(start+pos//4)*bs
        assert events.dtype==np.int64 and np.all((events>=0)&(events<limit))
        values=[x[events] for x in data]
        if row['direction']=='d':values[0],values[1]=values[1],values[0]
        exact=all(row[key].numpy().shape==value.shape and row[key].numpy().dtype==value.dtype and row[key].numpy().tobytes()==value.tobytes() for (key,_),value in zip(FIELDS,values))
        result.append(dict(call=pos,rows=len(events),events_before_current_batch=True,exact=exact))
    return dict(case=label,data=info,checks=result)
...
def main():
    ...
    result=dict(training=[],prefix=[],event_gathers=[],hashes={},cuda_initialized=False)
    for file in sorted((root/'artifacts').glob('*_tape.pt')):
        ...
        result['prefix'].append(dict(case=label,checks=prefix_check(records,checkpoint,reference)))
        result['event_gathers'].append(event_check(label,records))
    ...
    event_rows=[c for r in result['event_gathers'] for c in r['checks']]
    result['summary']['event_gathers']=dict(calls=len(event_rows),exact=all(c['exact'] for c in event_rows))
    assert len(event_rows)==384 and all(c['exact'] for c in event_rows)
