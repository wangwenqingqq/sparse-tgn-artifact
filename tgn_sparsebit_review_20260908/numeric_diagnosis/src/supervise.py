...
p.add_argument('--task', default='witnesses')
p.add_argument('--selected', default='compatible,time64')
args = p.parse_args()
...
env = dict(os.environ, CUDA_VISIBLE_DEVICES=gpu, PYTHONDONTWRITEBYTECODE='1',
           PYTHONPATH='/home/data/wangxuran/tncn_training_profile_20260908/deps', OMP_NUM_THREADS='1', OPENBLAS_NUM_THREADS='1', MKL_NUM_THREADS='1',
...
with open('/home/data/wangxuran/.locks/tgn_sptc_gpu2.lock', 'a+') as lock:
    try:
        ...
        cmd = ['/home/data/wangxuran/isaacsim6/env/bin/python', str(root/'src/diagnose.py'),
               '--output', str(out/'artifacts'), '--task', args.task, '--selected', args.selected]
        record['command'] = cmd
