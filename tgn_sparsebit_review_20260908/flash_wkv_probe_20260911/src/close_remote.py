...
m['gpu6_compute_processes_present_at_close']='GPU-865ae1f0-780e-d04c-5ec3-4deccea65f82' in m['processes_final']
process_listing=subprocess.check_output(['ps','-eo','pid,args'],text=True)
own_scripts=[str(ROOT/'src'/name) for name in ['capture.py','probe.py','probe_keep_empty.py','supervise.py','supervise_gpu6.py']]
m['active_experiment_processes']=[line for line in process_listing.splitlines() if any(script in line for script in own_scripts)]
assert not m['active_experiment_processes'],m['active_experiment_processes']
