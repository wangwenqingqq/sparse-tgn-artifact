...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--stage',choices=['qualify','noise'],required=True);args=ap.parse_args();assert not torch.cuda.is_initialized()
    root=ROOT/'output'/(args.stage+'_v1');art=root/'artifacts';meta=json.loads((ROOT/'output/capture_v1/artifacts/capture.json').read_text());hashes={};checked=0;passes=0
