...
def timing(out,meta):
    q=json.loads((a.ROOT/'output/qualify_v1/artifacts/qualification.json').read_text());audit=json.loads((a.ROOT/'output/qualify_v1/audit.json').read_text());assert q['all_pass'] and audit['all_pass']
    records=[]
    for port in meta['ports']:
...
def main():
    ...
    a.opening(args.stage,[Path(__file__),a.ROOT/'src/candidate.py',a.ROOT/'src/compare.py']);meta=json.loads((CAPTURE/'capture.json').read_text())
    monitor=c.Monitor()
